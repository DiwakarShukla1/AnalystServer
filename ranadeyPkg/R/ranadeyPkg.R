...
getCountByCropForAdvice=function(adviceData){
  return(aggregate(adviceData$count ~ adviceData$owner + adviceData$crop, data = adviceData, sum))
}

#----------------------------------------------Individual-------------------------------------------------

adviceDaysPatternOfEmp=function(param){
  library(rjson)
  param <- fromJSON(param)
  key = param$first
  adviceData = readAdvice()
  q=1
...
#___________________calculate employee added crop____________________________________________

cropAddedByEmployee=function(cropData){
...
#___________________calculate monthwise crop plantation______________________________________
...
#___________________calculate no of acre of crop____________________________________________
...
# farmerCountLastWeek=function(data){
#   data=aggregate(data$count ~ data$created_at, data = data, sum)
#   data=data[order(data$'data$created_at',decreasing = TRUE),]
#   data=head(data,7)
#   return(data)
...
farmerList=function(key,farmerData){
  ...
  while(i!=nrow(farmerData))
  {
    ...
    {
      data[[j]]=list('name'= farmerData[i,1],'date'= toString(farmerData[i,3]),'time'= farmerData[i,7], 'latitude'=  farmerData[i,4], 'longitude'= farmerData[i,5])
      j=j+1
    }
    i=i+1
  }
